...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let admin_addr =
        std::env::var("ADMIN_ADDR").unwrap_or_else(|_| "http://admin:8083".to_string());
    let custodian_addr = std::env::var("CUSTODIAN_ADDR")
        .unwrap_or_else(|_| "http://custodian-leader:8081".to_string());
    ...
    let jwt_secret =
        std::env::var("JWT_SECRET").map_or_else(|_| b"secret".to_vec(), String::into_bytes);
