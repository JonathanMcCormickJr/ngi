...
mod tests {
    ...
    async fn test_raft_service_handlers() {
        ...
        let raft = CustodianRaft::new(1u64, cfg, network_factory, log_store, state_machine).await.expect("create raft");
        ...
        let resp = svc.vote(tonic::Request::new(vote_req)).await.expect("vote rpc");
        ...
        let resp = svc.append_entries(tonic::Request::new(append_req)).await.expect("append rpc");
        ...
        let resp = svc.install_snapshot(tonic::Request::new(install_req)).await.expect("install snapshot rpc");
        assert_eq!(resp.get_ref().term, 1);
    }
}
...
impl RaftService for RaftServiceImpl {
    ...
    async fn install_snapshot(&self, request: Request<InstallSnapshotRequest>) -> Result<Response<InstallSnapshotResponse>, Status> {
        ...
        // Build SnapshotMeta
        let meta = openraft::SnapshotMeta { last_log_id: req.last_included_index.checked_sub(0).map(|index| LogId { leader_id: openraft::LeaderId { term: req.last_included_term, node_id: 0 }, index }), last_membership: openraft::StoredMembership::default(), snapshot_id: String::new() };
