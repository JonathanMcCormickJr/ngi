...
pub struct CustodianNetwork {
    _target: u64,
    address: String,
    client: Option<crate::server::custodian::raft_service_client::RaftServiceClient<tonic::transport::Channel>>,
}
...
mod tests {
    ...
    use crate::server::custodian::{VoteRequest, VoteResponse, AppendEntriesRequest, AppendEntriesResponse};
    ...
    impl RaftService for TestRaftSvc {
        async fn vote(&self, request: Request<VoteRequest>) -> Result<Response<VoteResponse>, Status> {
            ...
            Ok(Response::new(VoteResponse { term: req.term, vote_granted: true }))
        }

        async fn append_entries(&self, request: Request<AppendEntriesRequest>) -> Result<Response<AppendEntriesResponse>, Status> {
            let _req = request.into_inner();
            Ok(Response::new(AppendEntriesResponse { term: 1, success: true }))
        }

        async fn install_snapshot(&self, _request: Request<crate::server::custodian::InstallSnapshotRequest>) -> Result<Response<crate::server::custodian::InstallSnapshotResponse>, Status> {
            Ok(Response::new(crate::server::custodian::InstallSnapshotResponse { term: 1 }))
        }
    }
    ...
    async fn test_network_install_snapshot_multi_chunk() {
        ...
        impl RaftService for ChunkedSvc {
            async fn vote(&self, _request: Request<VoteRequest>) -> Result<Response<VoteResponse>, Status> {
                Ok(Response::new(VoteResponse { term: 1, vote_granted: true }))
            }
            ...
            async fn install_snapshot(&self, request: Request<InstallSnapshotRequest>) -> Result<Response<crate::server::custodian::InstallSnapshotResponse>, Status> {
                let req = request.into_inner();
                let mut guard = self.0.lock().unwrap();
                guard.extend_from_slice(&req.data);
                Ok(Response::new(crate::server::custodian::InstallSnapshotResponse { term: req.term }))
            }
        }
        ...
        let req1 = openraft::raft::InstallSnapshotRequest { 
            vote: openraft::Vote { leader_id: openraft::LeaderId { term: 1, node_id: 1 }, committed: true },
        ...
        // Send chunks
        let res1 = client.install_snapshot(req1, openraft::network::RPCOption::new(std::time::Duration::from_secs(1))).await;
        assert!(res1.is_ok());
        let res2 = client.install_snapshot(req2, openraft::network::RPCOption::new(std::time::Duration::from_secs(1))).await;
        assert!(res2.is_ok());
    ...
    async fn test_network_vote_and_append() {
        ...
        // Test vote
        let vote_req = openraft::raft::VoteRequest { vote: openraft::Vote { leader_id: openraft::LeaderId { term: 1, node_id: 1 }, committed: true }, last_log_id: None };
        let res = client.vote(vote_req, openraft::network::RPCOption::new(std::time::Duration::from_secs(1))).await;
        assert!(res.is_ok());

        // Test append
        let append_req = openraft::raft::AppendEntriesRequest { vote: openraft::Vote { leader_id: openraft::LeaderId { term: 1, node_id: 1 }, committed: true }, prev_log_id: None, entries: vec![], leader_commit: None };
        let res = client.append_entries(append_req, openraft::network::RPCOption::new(std::time::Duration::from_secs(1))).await;
        assert!(res.is_ok());
...
impl CustodianNetwork {
    async fn get_client(
        ...
        &mut crate::server::custodian::raft_service_client::RaftServiceClient<tonic::transport::Channel>,
