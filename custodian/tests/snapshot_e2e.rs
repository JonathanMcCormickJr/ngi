...
async fn test_snapshot_build_and_install_e2e() {
    ...
    leader_storage
        .put(TREE_LOCKS, b"lock-42", b"owner-xyz")
        .expect("put");

    // Build snapshot using the store's snapshot builder
    let mut leader_for_builder = leader.clone();
    let mut builder = <CustodianStore as RaftStorage<CustodianTypeConfig>>::get_snapshot_builder(
        &mut leader_for_builder,
    )
    .await;
    ...
    <CustodianStore as RaftStorage<CustodianTypeConfig>>::install_snapshot(
        &mut follower,
        &snapshot.meta,
        snapshot.snapshot,
    )
