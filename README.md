...
Each ticket in InfoVulcan contains the following fields:
...
### Key Components
- [**Admin:**](./admin/) Manages user accounts, roles, and permissions within the InfoVulcan system (introduced in Hardened stage).
...
- [**Tests:**](./tests/) Contains integration and end-to-end tests for the entire InfoVulcan system, ensuring that all components work together seamlessly.
...
### Consistency Model
InfoVulcan employs a flexible consistency model. Operations that critically rely on data consistency, such as setting/clearing ticket locks, utilize strong consistency to ensure data integrity. Operations that have no such requirement, such as UI format, maintain flexibility. For example, the UI can have A/B testing enabled, allowing different users to experience different UI layouts without impacting the underlying data consistency.

## Setup & Deployment
InfoVulcan is deployed as a collection of Nanos unikernels created using [NanoVMs's OPS tool](https://github.com/nanovms/ops). Docker is banned from this project due to attack surface concerns. Example usage of OPS can be found at https://github.com/nanovms/ops-examples/tree/master/rust. 

## CI/CD
Continuous integration and deployment (CI/CD) pipelines are set up using GitHub Actions to automate the build, test, and deployment processes for InfoVulcan. This ensures that new features and bug fixes are delivered quickly and reliably to users.
...
git clone https://github.com/JonathanMcCormickJr/infovulcan.git
cd infovulcan
...
curl -k https://service.infovulcan.internal:808x/health

# Check cluster status
curl -k https://admin.infovulcan.internal:8083/cluster/status

# View metrics
curl -k https://admin.infovulcan.internal:8083/metrics
```

### Log Access
```bash
# View recent logs
curl -k https://admin.infovulcan.internal:8083/logs?service=db&lines=100

# Search logs
curl -k "https://admin.infovulcan.internal:8083/logs/search?query=error&since=1h"
```

### Certificate Management
```bash
# Check certificate expiry
curl -k https://admin.infovulcan.internal:8083/certs/expiry

# Rotate certificates (automated)
curl -X POST -k https://admin.infovulcan.internal:8083/certs/rotate
```

### Backup & Recovery
```bash
# Create database snapshot
curl -X POST -k https://db-leader.infovulcan.internal:8080/admin/snapshot

# List available backups
curl -k https://admin.infovulcan.internal:8083/backups

# Restore from backup
curl -X POST -k https://db-leader.infovulcan.internal:8080/admin/restore \
...
curl -k https://admin.infovulcan.internal:8083/docs/troubleshooting

# Generate diagnostic report
curl -k https://admin.infovulcan.internal:8083/diagnostics > diagnostics.json
```
